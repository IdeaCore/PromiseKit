...
class JoinTests: XCTestCase {
    func testImmediates() {
        ...
        when(resolved: successPromise).then(on: zalgo) { _ in joinFinished = true }
        ...
        when(resolved: promise2, promise3, promise4).then(on: zalgo) { _ in join2Finished = true }
    ...
    func testImmediateErrors() {
        ...
        var joinFinished = false
        when(resolved: errorPromise).then(on: zalgo) { _ in joinFinished = true }
        ...
        var join2Finished = false
        when(resolved: errorPromise2, errorPromise3, errorPromise4).then(on: zalgo) { _ in join2Finished = true }
    ...
    func testFulfilledAfterAllResolve() {
        ...
        var finished = false
        when(resolved: promise1, promise2, promise3).then(on: zalgo) { _ in finished = true }
